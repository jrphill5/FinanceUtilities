...
class FinancePlot:
	...
	def genPlotTitle(self, fund):
		...
		self.ax.set_title('Thrift Savings Plan ' + fund + ' from ' + (self.t[len(self.t)-1]-timedelta(days=self.dd+1)).strftime("%m/%d/%Y") + ' to ' + self.t[len(self.t)-1].strftime("%m/%d/%Y"))
	...
	def plotSMASignals(self, tsp, t, p, img, fund, nl, nh):
		# Define datasets for analysis:
